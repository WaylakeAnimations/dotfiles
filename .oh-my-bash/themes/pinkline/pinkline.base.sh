  ...
  case "${POWERLINE_PROMPT_USER_INFO_MODE}" in
    ...
    *)
      if [[ -n "${SSH_CLIENT}" ]]; then
        user_info="${USER_INFO_SSH_CHAR}${USER}@${HOSTNAME} :3"
      else
        user_info="${USER} :3"
      fi
      ;;
...
function __powerline_scm_prompt {
  ...
  if [[ "${SCM_NONE_CHAR}" != "${SCM_CHAR}" ]]; then
    ...
    echo "${scm_prompt}${scm}|${color}|${SCM_GIT_TEXT_COLOR}"
  fi
}

function __powerline_cwd_prompt {
  echo "$(pwd | sed "s|^${HOME}|~|")|${CWD_THEME_PROMPT_COLOR}|${CWD_THEME_TEXT_COLOR}"
}
